...
$inicio = ($pagina-1)*$num_registros;
		...
		<table id="table" class="table table-striped" >
			...
			<tbody>
				<?php 
				 ...
				 if (isset($_GET['rol']) == true &&  $_GET['rol']  != "") {
					$filtros .= " AND permi.id_rol = '$_GET[rol]' ";
				}

			
				 if (isset($_GET['nombre_modulo']) == true &&  $_GET['nombre_modulo']  != "") {
					$filtros .= " AND permi.modulo = '$_GET[nombre_modulo]' ";
				}

		
				 if (isset($_GET['nombre_accion']) == true &&  $_GET['nombre_accion']  != "") {
					$filtros .= " AND permi.accion = '$_GET[nombre_accion]' ";
				}

		
					$s= "SELECT 
			permi.id_permiso_rol,
			permi.id_rol,
			r.nombre ,
			permi.modulo,
			permi.accion
			...
			permiso_rol  permi
			JOIN
			rol r ON permi.id_rol=r.id_rol
			...
			echo "
					<tr>
						...
						<td class='acciones'>
	                    <a href='javascript:;' class='mostrar'  onclick='mostrar(\"$rw[id_permiso_rol]\")'>
	                        <i class='fas fa-eye'></i>
	                    </a>
	                    <a href='javascript:;' class='eliminar' onclick='eliminar(\"$rw[id_permiso_rol]\")'>
	                        <i class='fas fa-trash'></i>
	                    </a>
	                    <a href='javascript:;' class='modificar' onclick='modificar(\"$rw[id_permiso_rol]\")'>
