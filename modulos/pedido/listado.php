...
$sql_cantidad = "SELECT count(*) as cantidad FROM pedido";
...
$inicio = ($pagina-1)*$num_registros;
		...
		<table id="table" class="table table-striped" >
			...
			<tbody>
		        ...
		        if (isset($_GET['id_pedido']) == true &&  $_GET['id_pedido']  != "") {
		            $filtros .= " AND p.id_pedido = '$_GET[id_pedido]' ";
		        }


		        if (isset($_GET['id_empleado']) == true &&  $_GET['id_empleado']  != "") {
		            $filtros .= " AND p.id_empleado = '$_GET[id_empleado]' ";
		        }	
		         ...
		         if (isset($_GET['id_persona']) == true &&  $_GET['id_persona']  != "") {
		          ...
		          if (isset($_GET['id_proveedor']) == true &&  $_GET['id_proveedor']  != "") {
		             $filtros .= " AND pr.id_proveedor = '$_GET[id_proveedor]' ";
		         }
 				
		         if (isset($_GET['fecha_creacion']) == true &&  $_GET['fecha_creacion']  != "") {
		             $filtros .= " AND p.fecha_creacion = '$_GET[fecha_creacion]' ";
		         }
		     
		      if (isset($_GET['fecha_vencimiento']) == true &&  $_GET['fecha_vencimiento']  != "") {
		             $filtros .= " AND p.fecha_vencimiento = '$_GET[fecha_vencimiento]' ";
		         }

				 if (isset($_GET['cantidad']) == true &&  $_GET['cantidad']  != "") {
		             $filtros .= " AND p.cantidad = '$_GET[cantidad]' ";
		         } 				
					...
					concat_ws(' ', prov.nombre, prov.apellido) as nombre_proveedor 
					FROM pedido as p inner JOIN empleado e
					on p.id_empleado = e.id_empleado inner join 
					...
					<tr>
						<td>$num</td> 						
						<td>$rw[nombre_empleado]</td>						
						<td>$rw[nombre_proveedor]</td>
						...
						<td class='acciones'>
	                    <a href='javascript:;' class='mostrar'  onclick='mostrar(\"$rw[id_pedido]\")'>
	                        <i class='fas fa-eye'></i>
	                    </a>
	                    <a href='javascript:;' class='eliminar' onclick='eliminar(\"$rw[id_pedido]\")'>
	                        <i class='fas fa-trash'></i>
	                    </a>
	                    <a href='javascript:;' class='modificar' onclick='modificar(\"$rw[id_pedido]\")'>
